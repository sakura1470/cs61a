...
def more_boar(player_score, opponent_score):
    ...
    if player_score < 10:
        ...
        player_max,player_min = max_min(player_score)
        if (player_max > opponent_score) & (player_min < opponent_score) :
            return True
        else :
            return False
    else:
        player_max,player_min = max_min(player_score)
        opponent_max,opponent_min = max_min(opponent_score)
        if (player_min < opponent_min) & (player_max > opponent_max) :
            return True
        else :
            return False
    # END PROBLEM 4
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if more_boar(score + piggy_points(opponent_score),opponent_score) or (not piggypoints_strategy(score,opponent_score,cutoff,num_rolls)):
        return 0
    else :
        return num_rolls  # Replace this statement
    # END PROBLEM 11
